...
**Candidate:** Phase 4 native desktop source candidate in this checkout. This walkthrough is based on the current UI and native command contracts, with synthetic browser IPC scenarios. It is not evidence of a rendered host app, signed or installed build, native dialog behavior, live Canvas access, or owner acceptance. No private coursework or screenshots are included.
...
Shows store/source status, fixed app-data folder, import time, refresh setting and last-refresh time, imported data classes, and privacy text. Preview explains that its data is a copy and offers `Replace preview copy…`. Only an authoritative store has the refresh preference toggle. It can remain enabled while unavailable; status explains that state, and no Refresh control is exposed until prerequisites pass.
...
The current evidence for this walkthrough is source inspection and synthetic fixtures/IPC scenarios. No host-rendered review or native picker interaction is claimed here. Signed packaging, installation, test-identifier GUI smoke, and production-identifier launch evidence remain separate Phase 4 gates. Synthetic Canvas mocks do not establish live Canvas behavior or a BWS invocation. Owner acceptance remains pending.
