/**
 * Desktop (Tauri) screens in a real browser, with Tauri IPC mocked in the page. No Tauri runtime is
 * involved: an init script installs `window.__TAURI_INTERNALS__` (invoke, transformCallback,
 * unregisterCallback) and `isTauri`, so `@tauri-apps/api/core` and the app's desktop code run
 * unchanged against synthetic command results. Every IPC call is recorded and checked for paths.
 */
...
const COMMANDS = ["store_status", "choose_legacy_root", "dry_run_import", "import_legacy_root", "read_dashboard_documents", "read_avatar_bytes", "open_library_resource", "copy_assignment_text", "set_item_completion", "set_discussion_field", "list_snapshots", "restore_snapshot", "export_legacy_folder", "set_canvas_refresh_enabled", "start_canvas_refresh", "prepare_store_promotion", "confirm_store_promotion", "demote_store_for_rollback", "export_frozen_for_rollback"];
...
function scenario(overrides: Partial<Scenario> = {}): Scenario {
  ...
  return {
    ...
    mutationConflicts: [], snapshotId: "daily-20260923T120000Z-0123abcd", copyFailure: false, canvasRefreshEnabled: false, helperReady: false,
...
async function expectNoPathArguments(page: Page): Promise<RecordedCall[]> {
  ...
  for (const { command, args } of calls) {
    ...
    const keysByCommand: Readonly<Record<string, readonly string[]>> = {
      ...
      set_canvas_refresh_enabled: ["enabled"], start_canvas_refresh: ["onProgress"],
...
for (const viewport of [{ name: "desktop", width: 1280, height: 800 }, { name: "mobile", width: 390, height: 844 }] as const) {
  test.describe(`desktop app screens at ${viewport.name} width`, () => {
    ...
    test("first run: fixed folder, folder picker, counts-only dry run, streamed import, writable preview", async ({ page }) => {
      ...
      await expect(page.getByRole("heading", { level: 1, name: "Set up local storage" })).toBeVisible();
      ...
      await expect(page.locator(".preview-label")).toContainText("It never refreshes or follows later changes in the browser app. Personal progress edits stay in this copy.");
    ...
    test("replace preview: keep the current copy, then archive and replace it", async ({ page }) => {
      ...
      await expect(card).toContainText("It never refreshes");
    ...
    test("configured native refresh streams progress and reports an incomplete run truthfully", async ({ page }) => {
      ...
      await page.getByRole("navigation", { name: "Primary" }).getByRole("link", { name: "More", exact: true }).click();
      await expect(page.locator("[data-desktop-store=authoritative]")).toContainText("Last Canvas refresh");
    ...
    test("rollback demotes first, disables refresh, then exports and verifies the frozen legacy source", async ({ page }) => {
      ...
      await expect(page.locator("[data-desktop-store=preview]")).toContainText("Frozen rollback export verified. Use this copy when restoring the legacy browser source.");
