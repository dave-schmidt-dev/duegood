...
function isDateOnly(value: string | number | null | undefined): value is string { return typeof value === "string" && /^\d{4}-\d{2}-\d{2}$/.test(value); }
function asDate(value: string | number | null | undefined): Date | undefined { if (value === null || value === undefined || value === "") return undefined; const date = new Date(isDateOnly(value) ? `${value}T00:00:00` : value); return Number.isNaN(date.getTime()) || isDateOnly(value) && dayKey(date) !== value ? undefined : date; }
function formatted(value: string | number | null | undefined, withTime = false): string { const date = asDate(value); return date === undefined ? "Unknown" : new Intl.DateTimeFormat("en-US", withTime && !isDateOnly(value) ? { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" } : { month: "short", day: "numeric" }).format(date); }
function time(value: string | number | null | undefined): string { if (isDateOnly(value)) return "Time not specified"; const date = asDate(value); return date === undefined ? "Time unknown" : new Intl.DateTimeFormat("en-US", { hour: "numeric", minute: "2-digit" }).format(date); }
...
function dayNumber(date: Date): number { return Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()) / 86_400_000; }
function eventDate(event: DashboardEvent): Date { return asDate(event.startsAt)!; }
function upcoming(value: string, now: number): boolean { const date = asDate(value); return date !== undefined && (isDateOnly(value) ? dayKey(date) >= dayKey(new Date(now)) : date.getTime() >= now); }
function overdue(value: string, now: number): boolean { const date = asDate(value); return date !== undefined && (isDateOnly(value) ? dayKey(date) < dayKey(new Date(now)) : date.getTime() < now); }
function eventTime(event: DashboardEvent): string { return isDateOnly(event.startsAt) ? "Time not specified" : `${time(event.startsAt)}${event.endsAt ? `–${time(event.endsAt)}` : ""}`; }
export function countdownText(now: number, deadline: string): string { const due = asDate(deadline); if (due === undefined) return "Time unknown"; if (isDateOnly(deadline)) { const days = dayNumber(due) - dayNumber(new Date(now)); return days < 0 ? "Due date passed" : days === 0 ? "Due today" : days === 1 ? "Due tomorrow" : `Due in ${String(days)} days`; } const minutes = Math.max(0, Math.floor((due.getTime() - now) / 60_000)); const days = Math.floor(minutes / 1440); const hours = Math.floor((minutes % 1440) / 60); const remainder = minutes % 60; return days > 0 ? `${days}d ${hours}h` : hours > 0 ? `${hours}h ${remainder}m` : `${remainder}m`; }
...
function locked(id: string, pendingIds: ReadonlySet<string>): boolean {
...
function completionButton(event: DashboardEvent, state: DashboardState, handlers: DashboardHandlers): ElementDescriptor {
  ...
  const disabled = locked(event.id, state.pendingCompletionIds);
...
function discussionCheckboxes(event: DashboardEvent, state: DashboardState, handlers: DashboardHandlers): readonly ElementDescriptor[] {
  ...
  const disabled = locked(event.id, state.pendingDiscussionIds);
...
export function formatAssignmentCopyText(event: DashboardEvent): string {
  const lines: string[] = [
    ...
    `Due ${isDateOnly(event.startsAt) ? "date" : "date/time"}: ${formatted(event.startsAt, true)}`,
  ];
...
function eventCard(event: DashboardEvent, state: DashboardState, handlers: DashboardHandlers): ElementDescriptor {
  ...
  return el("article", undefined, { class: `event-card ${event.kind === "class" ? "class-meeting" : ""} ${event.kind === "discussion" ? "discussion-card" : ""} ${event.completed ? "completed" : ""}`.trim(), style: `--course-color:${color(state.data.courses, event.courseCode)}` }, [
    el("div", undefined, { class: "event-top" }, [el("div", undefined, undefined, [el("div", `${code(event.courseCode)} · ${event.kind === "class" ? "Class meeting" : event.kind === "discussion" ? "Discussion" : "Assignment due"}`, { class: "event-kind" }), el("h2", event.title, { class: "event-title" })]), el("time", eventTime(event), { class: "event-time", datetime: event.startsAt })]),
...
function dueRailItem(event: DashboardEvent, state: DashboardState, handlers: DashboardHandlers): ElementDescriptor {
  ...
  const isOverdue = overdue(event.startsAt, state.now);
  const meta = [el("span", isOverdue ? "Overdue" : `Due ${formatted(event.startsAt, true)}`), ...(event.points === null || event.points === undefined ? [] : [el("span", `${String(event.points)} pts`)]), el("span", isOverdue ? "Needs attention" : countdownText(state.now, event.startsAt), { class: isOverdue ? "rail-countdown overdue" : "rail-countdown" })];
...
function timelinePage(state: DashboardState, handlers: DashboardHandlers): ElementDescriptor {
  ...
  const events = state.data.events.filter((event) => !event.completed && (state.eventMode === "all" || event.kind !== "class") && (state.courseFilter === "all" || key(event.courseCode) === key(state.courseFilter)) && asDate(event.startsAt) !== undefined).sort((a, b) => eventDate(a).getTime() - eventDate(b).getTime());
  const dueItems = state.data.events.filter((event) => event.kind !== "class" && !event.completed && asDate(event.startsAt) !== undefined).sort((a, b) => eventDate(a).getTime() - eventDate(b).getTime());
  ...
  const next = state.data.events.filter((event) => event.kind !== "class" && !event.completed && upcoming(event.startsAt, state.now)).sort((a, b) => eventDate(a).getTime() - eventDate(b).getTime())[0];
  const start = startOfDay(state.now);
  const minimumEnd = new Date(start); minimumEnd.setDate(minimumEnd.getDate() + 6);
  const latest = events.reduce((maximum, event) => Math.max(maximum, eventDate(event).getTime()), start.getTime());
...
function gradeEditorCell(event: DashboardEvent, state: DashboardState, handlers: DashboardHandlers): ElementDescriptor {
  if (!/^[a-f0-9]{64}$/.test(state.data.version)) return el("td", "Local editing unavailable");
  ...
  const list = resources.length === 0
    ...
    : el("div", undefined, { class: "resource-list" }, resources.map((resource) => {
      ...
      return el("article", undefined, { class: "resource-row", style: `--course-color:${color(state.data.courses, resource.courseCode)}` }, [
        ...
        ...(resource.savedLocally === true && handlers.onOpenResource !== undefined
...
function desktopStoreCard(desktop: DesktopStoreInfo, state: DashboardState, handlers: DashboardHandlers): ElementDescriptor {
  ...
  const transitionDetails: ElementDescriptor[] = preview
    ? [
      ...
      ...(handlers.onExportFrozenRollback === undefined ? [] : [
        ...
        ...(transition.rollbackExportVerified === true ? [el("p", "Frozen rollback export verified. Use this copy when restoring the legacy local source.", { class: "status", role: "status" })] : []),
      ]),
    ]
    : [
  ...
  return el("section", undefined, { class: "settings-card", "data-desktop-store": desktop.storeState }, [
    ...
    el("p", preview ? "A copy imported from the legacy local source. It does not follow later source changes. Personal progress edits stay here until cutover." : "This app store is the authoritative copy. Import never replaces it."),
...
function pendingLinkReview(state: DashboardState, handlers: DashboardHandlers): ElementDescriptor | undefined {
  ...
  const canKeepDistinct = selected.needsRefresh !== true;
...
function morePage(state: DashboardState, handlers: DashboardHandlers): ElementDescriptor {
  const pendingReview = pendingLinkReview(state, handlers);
  return el("section", undefined, { class: "page" }, [
    heading("Due Good", "More", "Source status, refresh controls, and local app information."),
    ...(state.pendingLinkNotice === undefined ? [] : [el("p", state.pendingLinkNotice, { class: "status", role: "status", "aria-live": "polite" })]),
    ...(pendingReview === undefined ? [] : [pendingReview]),
    el("div", undefined, { class: "more-grid" }, [
      desktopStoreCard(state.desktop, state, handlers),
      el("section", undefined, { class: "settings-card" }, [
        el("h2", "Import behavior"),
        el("p", "Incomplete refreshes never remove existing coursework."),
        el("div", undefined, { class: "settings-row" }, [el("div", undefined, undefined, [el("strong", "Canvas items"), el("span", "Assignments, class meetings, files, pages, links, modules, announcements, and read-only inbox")])]),
